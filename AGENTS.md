...
- The primary checkout is `H:\Koding\MeshVenes` on Windows 11. Use PowerShell and run commands from the repository root.
...
- If a task genuinely requires Mac work, cross-machine transfer, or Syncthing, read `H:\Koding\utvikling.md` first and follow it. That file is private and must never be copied into this repository or committed.
...
- Create a release only when the user explicitly requests publication. Follow the private release safeguards in `H:\Koding\utvikling.md` without copying that file into the repository.

Release flow:

1. Bump the version and complete the build and tests locally on Windows.
2. Publish to a temporary directory outside `H:\Koding\Venes.org`.
...
6. Copy the verified ZIP and `version.json` directly to `H:\Koding\Venes.org\meshvenes`, then verify the copied size and SHA-256.
7. WinSCP uploads changes from that folder automatically. Confirm the public manifest and ZIP before reporting success.

- Do not use `H:\Koding\venes-upload` or a GitHub Actions `venes-upload` artifact.
