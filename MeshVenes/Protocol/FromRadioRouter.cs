...
public static class FromRadioRouter
{
    ...
    private static void ApplyPacketObject(object packetObj, Action<string> logToUi)
    {
        ...
        if (TryHandleMeshAppFromPayload(portNum, decodedObj, fromNodeNum, logToUi, out var meshAppSummary))
        {
        ...
        if (packetId != 0)
        {
            var existingByPacket = MeshVenes.AppState.Messages.FirstOrDefault(m =>
                !m.IsMine &&
                m.PacketId == packetId &&
        ...
        var toNode = MeshVenes.AppState.Nodes.FirstOrDefault(n => string.Equals(n.IdHex, toIdHex, StringComparison.OrdinalIgnoreCase));
        ...
        MeshVenes.AppState.NotifyIncomingMessage(msg.IsDirect ? fromIdHex : msg.ChannelPeerKey, msg.WhenUtc);

        MeshVenes.AppState.Messages.Insert(0, msg);
        while (MeshVenes.AppState.Messages.Count > MaxMessages)
            MeshVenes.AppState.Messages.RemoveAt(MeshVenes.AppState.Messages.Count - 1);
    ...
    private static void ReplaceIncomingMessageText(
        ...
        string channelName)
    {
        var index = MeshVenes.AppState.Messages.IndexOf(existing);
        if (index < 0)
            return;
        ...
        MeshVenes.AppState.Messages[index] = updated;
    }
        ...
        string channelName)
    {
        ...
        var candidate = MeshVenes.AppState.Messages
            .Where(m =>
                !m.IsMine &&
                m.PacketId == 0 &&
                string.Equals(m.FromIdHex, fromIdHex, StringComparison.OrdinalIgnoreCase) &&
    ...
    private static void ApplyDeviceMetadataObject(object metadataObj)
    {
        ...
        if (node is null && MeshVenes.AppState.Nodes.Count == 1)
            node = MeshVenes.AppState.Nodes[0];
    ...
    {
        var node = MeshVenes.AppState.Nodes.FirstOrDefault(n => string.Equals(n.IdHex, idHex, StringComparison.OrdinalIgnoreCase));
        if (node is null)
        {
            node = new NodeLive(idHex);
            MeshVenes.AppState.Nodes.Insert(0, node);
        }
        ...
        out string summary)
    {
        ...
        else
        {
            ...
            {
                var connected = MeshVenes.AppState.Nodes.FirstOrDefault(n =>
                    string.Equals(n.IdHex, MeshVenes.AppState.ConnectedNodeIdHex, StringComparison.OrdinalIgnoreCase));
    ...
    private static bool TryHandleMeshAppFromPayload(int portNum, object decodedObj, uint fromNodeNum, Action<string> logToUi, out string summary)
    {
        summary = "";
        var payloadBytes = TryGetPayloadBytes(decodedObj);
        if (payloadBytes is null || payloadBytes.Length == 0)
