...
public sealed partial class MapsPage : Page, INotifyPropertyChanged
{
    ...
    private static bool TryParseWaypointUpsertRequest(JsonElement root, out WaypointUpsertRequest request)
    {
        ...
        if (!root.TryGetProperty("waypoint", out var waypointElement) || waypointElement.ValueKind != JsonValueKind.Object)
            return false;

        _ = TryReadUInt(waypointElement, "id", out var waypointId);
        if (!TryReadDouble(waypointElement, "lat", out var latitude)
            || !TryReadDouble(waypointElement, "lon", out var longitude))
        {
            return false;
        }

        var name = TryReadString(waypointElement, "name");
        var description = TryReadString(waypointElement, "description");
        var iconGlyph = TryReadString(waypointElement, "iconGlyph");

        _ = TryReadUInt(waypointElement, "lockedTo", out var lockedTo);

        long? expireUnix = null;
        if (TryReadLong(waypointElement, "expireUnix", out var parsedExpire) && parsedExpire > 0)
            expireUnix = parsedExpire;

        _ = TryReadUInt(waypointElement, "channel", out var channel);

        request = new WaypointUpsertRequest(
            waypointId,
            latitude,
            longitude,
            name,
    ...
    private async System.Threading.Tasks.Task HandleWaypointUpsertRequestAsync(WaypointUpsertRequest request)
    {
        ...
        var payload = new Waypoint
        {
            ...
            Name = LimitText(string.IsNullOrWhiteSpace(request.Name) ? $"Waypoint {waypointId:x8}" : request.Name, 30),
            Description = LimitText(request.Description, 100),
            LockedTo = request.LockedToNodeNum,
            Icon = WaypointLive.ParseCodepointFromGlyph(request.IconGlyph)
        };

        if (request.ExpireUnixUtc.HasValue)
        ...
        var sourceNodeNum = _selected is { NodeNum: > 0 } ? (uint)_selected.NodeNum : 0u;
    ...
    private static bool TryReadUInt(JsonElement root, string propertyName, out uint value)
    {
        ...
        return element.ValueKind == JsonValueKind.String
            && uint.TryParse(element.GetString(), NumberStyles.Integer, CultureInfo.InvariantCulture, out value);
    }
