...
public sealed partial class MessagesPage : Page, INotifyPropertyChanged
{
    ...
    private void HookAppStateEvents()
    {
        ...
        MeshVenes.AppState.Messages.CollectionChanged += Messages_CollectionChanged;
    ...
    {
        var connectedNodeIdHex = MeshVenes.AppState.ConnectedNodeIdHex;
        ...
        _ = dq.TryEnqueue(() =>
        {
            if (!string.Equals(MeshVenes.AppState.ConnectedNodeIdHex, connectedNodeIdHex, StringComparison.OrdinalIgnoreCase))
                return;
    ...
    private void UpdateUnreadIndicators(string? peerIdHex)
    {
        ...
        {
            _primaryChatItem.UnreadVisible = MeshVenes.AppState.HasUnread(null)
                ? Visibility.Visible
        ...
        if (_chatItemsByPeer.TryGetValue(peerIdHex, out var item))
            item.UnreadVisible = MeshVenes.AppState.HasUnread(peerIdHex)
                ? Visibility.Visible
                : Visibility.Collapsed;
    }
    ...
    {
        if (sender is not FrameworkElement element || element.DataContext is not ChatListItemVm chat)
            return;

        if (chat.IsChannel || string.IsNullOrWhiteSpace(chat.PeerIdHex))
            return;

        var node = MeshVenes.AppState.Nodes.FirstOrDefault(n =>
            string.Equals(n.IdHex, chat.PeerIdHex, StringComparison.OrdinalIgnoreCase));
        if (node is null || node.NodeNum <= 0 || node.NodeNum > uint.MaxValue)
            return;

        if (!NodeIdentity.TryParseNodeNumFromHex(AppState.ConnectedNodeIdHex, out var adminNodeNum))
            return;

        var desiredFavorite = !chat.IsFavorite;
        ...
        try
        {
            ...
            chat.IsFavorite = desiredFavorite;
            RebuildVisibleChats();
        }
        catch
        {
            // Ignore failures to keep chat list responsive.
        }
        finally
        {
    ...
    private void AddMessageToChat(MessageLive message)
    {
        ...
        chat.HasLogIndicator = !ReferenceEquals(chat, _selectedChatItem);
    ...
    private async System.Threading.Tasks.Task SendNowAsync()
    {
        ...
        MeshVenes.AppState.Messages.Insert(0, local);
...
public sealed class ChatListItemVm : INotifyPropertyChanged
{
    ...
    public Visibility FavoriteVisibility
    {
        ...
        set { if (_favoriteVisibility != value) { _favoriteVisibility = value; OnChanged(nameof(FavoriteVisibility)); } }
    }

    private string _hopsBadgeText = "";
    public string HopsBadgeText
    {
        get => _hopsBadgeText;
